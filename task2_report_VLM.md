...
Since MedGemma is a generative Visual Language Model, its performance depends heavily on how the task is described in natural language. In this context, prompting acts as a form of linguistic guidance - meaning the wording, structure, and constraints of the prompt directly influence how the model interprets the image and formats its response. Three prompting strategies were tested, each representing a different level of linguistic constraint and clinical structuring. The goal was to evaluate how varying degrees of language guidance affect diagnostic accuracy and report quality.
